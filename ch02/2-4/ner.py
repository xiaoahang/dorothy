...
fin = open(os.path.join(project_base_dir, './ch02/2-4/news.txt'), 'r', encoding='utf8')
fner = open(os.path.join(project_base_dir, './ch02/2-4/ner.txt'), 'w', encoding='utf8')  # 实体
ftag = open(os.path.join(project_base_dir, './ch02/2-4/pos_tag.txt'), 'w', encoding='utf8')  # 词性
for line in fin:
    line = line.strip()
    if len(line) < 1:  # 空行
        continue
